export 'package:todo_app/res/colors.dart';
export 'package:todo_app/controllers/tasks_controller.dart';

// widgets
export 'package:todo_app/res/widgets/text_fields.dart';
export 'package:todo_app/res/widgets/button.dart';

// Screens
export 'package:todo_app/view/auth/splash_screen.dart';
export 'package:todo_app/view/auth/login_screen.dart';
export 'package:todo_app/view/auth/sign_up.dart';
export 'package:todo_app/view/home_screen.dart';

// Packages
export 'package:flutter/material.dart';
export 'package:get/get.dart';
export 'package:flutter_screenutil/flutter_screenutil.dart';
export 'package:google_fonts/google_fonts.dart';
export 'package:image_picker/image_picker.dart';
export 'package:flutter_zoom_drawer/flutter_zoom_drawer.dart';
export 'package:percent_indicator/percent_indicator.dart';
export 'package:flutter_spinkit/flutter_spinkit.dart';
export 'package:flutter_secure_storage/flutter_secure_storage.dart';

//utils => Routes
export 'package:todo_app/utils/utils.dart';
export 'package:todo_app/utils/routes/routes.dart';
export 'package:todo_app/utils/routes/route_names.dart';

//data
export 'package:todo_app/data/network/base_api_services.dart';
export 'package:todo_app/data/app_exception.dart';
export 'package:todo_app/data/network/network_api_service.dart';

// res
export 'package:todo_app/res/app_urls.dart';
export 'package:todo_app/res/global.dart';
export 'package:todo_app/res/export_all.dart';

// repository
export 'package:todo_app/repository/auth_repo.dart';

//services
export 'package:todo_app/services/local_storage.dart';
export 'package:todo_app/services/helper_functions.dart';

// models
export 'package:todo_app/model/user_model.dart';
